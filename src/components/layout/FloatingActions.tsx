 ...
 * زر واتساب العائم الذكي — مخصص للهواتف بدون تداخل مع عناصر الصفحة
 */

import { useState, useEffect } from 'react'
import Link from 'next/link'
import { PhoneIcon, CalendarDaysIcon, XMarkIcon } from '@heroicons/react/24/solid'
...
export default function FloatingActions() {
  ...
  // Show FAB after scrolling 150px
  useEffect(() => {
    const onScroll = () => setIsVisible(window.scrollY > 150)
  ...
  return (
    <div
      className="fixed z-40 flex flex-col items-center gap-2.5 no-print"
      style={{
        bottom: 'max(1.25rem, calc(env(safe-area-inset-bottom, 0px) + 0.5rem))',
        insetInlineEnd: '1.25rem', // Left in RTL, right in LTR — avoids covering Arabic content
      }}
      role="complementary"
    ...
    >
      ...
      <div className="relative flex items-center justify-center">
        {/* Toggle options button (small badge above or long press) */}
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className="absolute -top-2 -start-2 w-6 h-6 rounded-full bg-navy-800 text-white border-2 border-white flex items-center justify-center text-xs font-black shadow-md z-10 hover:bg-navy-900 transition-colors"
